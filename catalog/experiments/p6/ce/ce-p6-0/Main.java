...
public class Main {
    ...
    public static void experiment(int repetitions, String out) throws JSONException {
        ...
        for (int i = 0; i < repetitions; i++) {
            double itemPrice = 10 + (i % 91); // Price between 10 and 100
            int quantity = 1 + (i % 5); // Quantity between 1 and 5

            double subtotal = itemPrice * quantity;
            double tax = subtotal * TAX_RATE;
            double discountAmount = subtotal >= DISCOUNT_THRESHOLD ? subtotal * DISCOUNT_RATE : 0;
            double total = subtotal + tax - discountAmount;

            totalRevenue += total;
        }
        System.out.println("With Temp Variables Total Revenue: " + totalRevenue);
